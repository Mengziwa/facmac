...
class CQMixMAC(BasicMAC):
    ...
    def select_actions(self, ep_batch, t_ep, t_env, bs=slice(None), test_mode=False, past_actions=None, critic=None,
        ...
        #    chosen_actions = self.forward(inputs, t_ep,
        #                                  test_mode=test_mode,
        #                                  select_actions=True)["actions"] # just to make sure detach
        #    chosen_actions = chosen_actions.view(ep_batch[bs].batch_size, self.n_agents, self.args.n_actions).detach()
        #   pass
    ...
    def _build_G(self, batch, t, target_mac=False, last_target_action=None):
        bs = batch.batch_size
        H = []
        H.append(batch["adj"][:, t])  # b1av
        H = th.cat([x.reshape(bs * self.n_agents, -1) for x in H], dim=1)
        G = generate_G_from_H(H.cpu().numpy())
        G = th.tensor(G).cuda().to(th.float32)
        return G
